...
def get_valid_timestamp():
    while True:
        ...
        try:
            # 입력된 시간의 유효성 검사
            year, month, day = int(user_input[0:4]), int(user_input[5:7]), int(user_input[8:10])
            hour, minute, second = int(user_input[11:13]), int(user_input[14:16]), int(user_input[17:19])
...
def sort_log_file():
    try:
        with open("./w3/data/processed/mars_mission_log.txt", "r") as log_file:
            log_entries = log_file.read().strip().split("\n\n")

        log_entries.sort(key=lambda entry: entry.split("\n")[0].strip("[]"))

        with open("./w3/data/processed/mars_mission_log.txt", "w") as log_file:
            log_file.write("\n\n".join(log_entries) + "\n\n")
    except IOError as e:
